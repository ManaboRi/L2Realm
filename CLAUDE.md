...
- **Full SSR для `/servers/[id]`** — после деплоя проверить `curl -s https://l2realm.ru/servers/ID | grep -oE "<h1|<p|<meta"` — имя сервера, описание, отзывы должны быть в HTML ответа
