...
### Монетизация
- [x] **VIP** — 5 слотов, 5 000 ₽ / 31 день. Отдельный блок на главной, золотая обводка. Когда мест нет — видна дата ближайшего освобождения.
- [x] **Буст 🔥** — 500 ₽ / 7 дней. Поднимает сервер выше обычных в каталоге, анимация огонька. Продление суммируется.
- [x] **Сервер недели ★** — бесплатно. Проект с максимумом голосов за текущую неделю, подсвечивается изумрудом.
...
### Админ-панель
- [x] CRUD серверов + статус (VIP/Буст/сервер недели/ручной online override) прямо в таблице
- [x] Модерация отзывов
- [x] Модерация заявок на добавление
- [x] Вкладка «Монетизация»: 5 VIP-слотов на главной, 5 VIP-слотов «Скоро открытие», таблица активных/истёкших бустов, инфо-карточка «Сервер недели»
...
- `POST /api/payments/purchase` — купить VIP, буст или VIP в «Скоро открытие» (`{ kind, serverId, returnUrl }`). Email плательщика берётся из JWT и уходит в чек ЮКассы.
- `GET  /api/payments/vip/status` — занято VIP-мест, ближайшая дата освобождения
- `GET  /api/payments/vip/soon-status` — занято VIP-мест в «Скоро открытие», ближайшая дата освобождения
...
| 🔥 Буст | Поднять над обычными + огонёк и анимация | 7 дней | 500 ₽ | Покупка в любой момент, продление суммируется |
| ◆ VIP | Отдельный блок «VIP Серверы» на главной, золотая обводка | 31 день | 5 000 ₽ | Всего 5 мест, FIFO |
| ⏳ Скоро открытие | Анонс сервера до запуска в отдельном разделе | до даты открытия | 500 ₽ | После оплаты уходит на модерацию |
| ◆ VIP Скоро | Отдельный VIP-блок в «Скоро открытие» для конкретного будущего запуска | 31 день | 2 000 ₽ | Всего 5 мест |
| ★ Сервер недели | Проект с максимумом недельных голосов, изумрудная подсветка | неделя | Бесплатно | По голосам текущей недели |

> VIP → по истечении сервер возвращается во FREE (cron). Буст не продляется автоматически.
> «Сервер недели» берётся из `weeklyVotes`; месячный сброс также начинает новую борьбу за топ.
